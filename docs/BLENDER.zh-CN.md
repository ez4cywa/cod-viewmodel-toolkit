...
3.5.0 提供 `cod-viewmodel-toolkit-3.5.0-blender-zh-CN.zip`。本版集成改进针对 Maya；
Blender 保留项目补丁版 CAST 2.00 后端及原有导入／导出算法，仅同步安装包版本，
没有新增 Blender 功能。中文包翻译插件面板、
...
中文包包含与英文 3.5.0 相同的米制输出功能。

[English](BLENDER.md) · [项目首页](../README.zh-CN.md)

要求 Blender 5.2+，旧版已在 Windows 官方 Blender 5.2.1 LTS 中验证。可选中英文侧栏，
不依赖 Maya。发布包不包含游戏素材、Blender 程序或 Maya 运行环境。

## 安装与使用

1. 下载 `cod-viewmodel-toolkit-3.5.0-blender-zh-CN.zip`（中文）或 `cod-viewmodel-toolkit-3.5.0-blender-en.zip`（英文）。
