...
def importSkeletonNode(name, skeleton, collection):
    ...
    for i, bone in enumerate(bones):
        ...
        if bone.SegmentScaleCompensate():
...
def importModelNode(self, model, path, selectedObject):
    ...
    # CoD toolkit patch: let model+animation files target their newly imported
    # armature instead of whichever unrelated object happened to be active.
    return selectedObject if self.import_merge and selectedObject else skeletonObj
...
def importCast(self, context, path):
    ...
    for root in cast.Roots():
        ...
        for child in root.ChildrenOfType(Model):
            imported = importModelNode(self, child, path, selectedObject)
            if imported is not None and imported.type == 'ARMATURE':
                importedArmatures.append(imported)
        animationTarget = selectedObject
        if root.ChildrenOfType(Animation) and importedArmatures:
            if len(importedArmatures) != 1:
                raise ValueError("Model+animation CAST requires one target armature")
            animationTarget = importedArmatures[0]
            bpy.context.view_layer.objects.active = animationTarget
            animationTarget.select_set(True)
        for child in root.ChildrenOfType(Animation):
            importAnimationNode(self, child, path, animationTarget)
