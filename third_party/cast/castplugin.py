...
SANITIZE_SUB = re.compile(r"[^A-Za-z0-9_]").sub
...
def exportModel(root, exportSelected, filePath):
    ...
    for i in xrange(objects.length()):
        ...
        while not vertexIter.isDone():
            ...
            for i, uvLayer in enumerate(uvLayers):
                try:
                    ...
                    missingUV = missingUVs.setdefault(uvLayer, [0, []])
                    missingUV[0] += 1
                    if len(missingUV[1]) < 5:
                        missingUV[1].append(index)

                    # CAST stores UVs per vertex. Maya permits a vertex to have
                    # no UV assignment, so keep the vertex and use the flipped
                    # representation of Maya UV (0, 0) as a safe default.
                    vertexUVLayers[i][index] = (0.0, 1.0)
                    continue
        ...
        for uvLayer, missingUV in missingUVs.items():
            cmds.warning(
                "Mesh %s has %s vertices without UVs in set %s "
                "(first indices: %s); exported those UVs as (0, 0)." %
                (meshPath, missingUV[0], uvLayer,
                 ", ".join(str(index) for index in missingUV[1])))
...
def utilityApplyTranslatorOptions(optionString, supportedNames):
    """Apply explicit per-call booleans and return values to restore."""
    previous = {}
    for option in str(optionString or "").split(";"):
        if "=" not in option:
            continue
        name, value = option.split("=", 1)
        name = name.strip()
        if name not in supportedNames or name not in sceneSettings:
            continue
        previous[name] = sceneSettings[name]
        sceneSettings[name] = value.strip().lower() in (
            "1", "true", "yes", "on")
    return previous


def utilityRestoreTranslatorOptions(previous):
    """Restore values returned by utilityApplyTranslatorOptions."""
    for name, value in previous.items():
        sceneSettings[name] = value
...
class CastFileTranslator(OpenMayaMPx.MPxFileTranslator):
    ...
    def writer(self, fileObject, optionString, accessMode):
        # Maya passes per-export translator options as a semicolon-delimited
        # string. Honour explicit model/animation choices for scripted exports
        # without persisting them to cast.cfg or the interactive menu.
        previous = utilityApplyTranslatorOptions(
            optionString, ("exportModel", "exportAnim", "bakeKeyframes"))

        try:
            exportCast(fileObject.fullName(), exportSelected=accessMode ==
                       OpenMayaMPx.MPxFileTranslator.kExportActiveAccessMode)
        finally:
            utilityRestoreTranslatorOptions(previous)

    def reader(self, fileObject, optionString, accessMode):
        previous = utilityApplyTranslatorOptions(
            optionString, (
                "importIK", "importConstraints", "importAtTime",
                "importReset", "importLooping"))
        try:
